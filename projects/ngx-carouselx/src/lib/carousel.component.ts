import { Component, ElementRef, EventEmitter, Inject, Input, Output, PLATFORM_ID, Renderer2, ViewChild, DoCheck, NgZone, OnInit, OnDestroy, ChangeDetectionStrategy, ChangeDetectorRef, OnChanges, SimpleChanges, AfterViewInit, makeStateKey, TransferState } from '@angular/core';
// import { SwipeService } from './swipe.service';
import { isPlatformServer, DOCUMENT, NgStyle } from '@angular/common';
...
export class CarouselComponent implements OnInit, AfterViewInit, DoCheck, OnChanges, OnDestroy {
  slideIndex: number = -1;
  slides: ISlide[] = [];
  ...
  ngOnInit() {
    if (this.debug !== undefined) {
      console.warn('[Deprecation Warning]: The debug input will be removed from ng-carousel in 1.3.0');
    }
  ...
  ngOnDestroy() {
    try {
      ...
      console.warn('Slide Subscription error caught in ng-carousel OnDestroy:', error);
    }
    ...
    try {
      ...
      console.warn('Click Subscription error caught in ng-carousel OnDestroy:', error);
    }
    ...
    try {
      ...
      console.warn('Pointer Service unbind error caught in ng-carousel OnDestroy:', error);
    }
    ...
    try {
      ...
      console.warn('Autoplay cancel error caught in ng-carousel OnDestroy:', error);
    }
  }
  ...
  private _onClick(): void {
    ...
    if(currentSlide) this.onClick.emit({ slide: currentSlide, index: this.slideIndex });
